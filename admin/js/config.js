...
var BarbWireSecurity = function () {
    this.switch_end_points = function () {
        var $ = jQuery;
        if (!$('input[name=specified_end_point]').is(':checked')) {
            $('textarea[name=end_points]').attr('readonly', 'readonly');
        } else {
            $('textarea[name=end_points]').removeAttr('readonly');
        }
    }
};

var barbwire_security = new BarbWireSecurity();
...
jQuery(function ($) {
    ...
    $('input[name=specified_end_point]').on('change', function () {
        barbwire_security.switch_end_points();
    });
    barbwire_security.switch_end_points();

    $('input.endpoint').on('click', function (e) {
        var target = jQuery(e.target);
        console.log(target);
        target.select();
    });

    $('#toggle_examples').on('click', function (e) {
